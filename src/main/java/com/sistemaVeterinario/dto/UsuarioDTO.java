...
public class UsuarioDTO {
    ...
    public @Pattern(regexp = "^(?!\\s)([a-zA-ZáéíóúÁÉÍÓÚñÑ\\s]{3,50})(?<!\\s)$", message = "{error.lastname.invalid}") String getApellido() {
        return apellido;
    }

    public void setApellido(@Pattern(regexp = "^(?!\\s)([a-zA-ZáéíóúÁÉÍÓÚñÑ\\s]{3,50})(?<!\\s)$", message = "{error.lastname.invalid}") String apellido) {
    ...
    public void setEmail(@Pattern(regexp =
            "^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$",
            message = "{error.email.invalid}"
    ) String email) {
        this.email = email;
